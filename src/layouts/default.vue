<template>
	<v-app>
		<v-app-bar :elevation="2" :height="appBarHeight">
			...
			<div id="title" class="ms-3 me-1 text-title-large text-truncate machine-name"
				 :title="machineName">
				{{ machineName }}
			</div>
			...
			<CodeInput v-if="isMdAndUp" class="mx-3 flex-grow-0 flex-shrink-0 code-input" />

			<v-spacer />

			<UploadButton v-if="lgAndUp" class="ms-3" />

			<EmergencyButton class="ms-3 me-2" />
		</v-app-bar>
...
<style scoped>
/* Cap the hostname width so a long machine name (anything past the available app-bar slot)
   ellipses instead of pushing the Emergency Stop button off-screen */
.machine-name {
	min-width: 0;
	max-width: 12rem;
}
