...
export default class HeightMapViewer {
	...
	drawLegend(canvas: HTMLCanvasElement, colorScheme: string, invertZ: boolean, xLabel: string, yLabel: string): void {
		...
		context.fillText(i18n.t('plugins.heightmap.scale') as string, canvas.width / 2, 21);
		context.fillText(`${invertZ ? -this.maxVisualizationZ : this.maxVisualizationZ} mm`, canvas.width / 2, 44);
		context.fillText(i18n.t(invertZ ? 'plugins.heightmap.orLess' : 'plugins.heightmap.orMore') as string, canvas.width / 2, 60);
		...
		if (colorScheme === 'terrain') {
			...
			context.fillText(i18n.t(invertZ ? 'plugins.heightmap.orMore' : 'plugins.heightmap.orLess') as string, canvas.width / 2, scaleHeight + 98);
			scaleHeight += 16;
		} else {
		...
		// Add axes
		if (showAxes) {
			context.fillText(i18n.t('plugins.heightmap.axes') as string, canvas.width / 2, scaleHeight + 109);
