<template>
	<v-row justify="center" align="center">
		<v-col class="shrink" :style="[backgroundColorStyle]">
			<v-text-field v-model="internalTextColor" @blur="updateValue(internalTextColor)"
						  @keyup.enter="updateValue(internalTextColor)" hide-details class="ma-0 pa-0" solo>
					...
					<v-menu v-model="menu" top nudge-bottom="105" nudge-left="16" :close-on-content-click="false">
						...
						<v-card>
							<v-card-text class="pa-0">
								<v-color-picker class="index-placement" v-model="color" flat
												@input="updateValue(color)">
								</v-color-picker>
							</v-card-text>
						</v-card>
					</v-menu>
