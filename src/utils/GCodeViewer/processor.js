...
processor.prototype.processGcodeFile = function(file){
...
processor.prototype.processLine = function(tokenString){
  ...
  switch(tokens[0]){
    ...
    for(var tokenIdx = 1; tokenIdx < tokens.length; tokenIdx++)
    {
      ...
      switch(token[0]){
        case "E":
        extruderPercentage = token.substring(1).split(":");
        break;
      }
    }
    for(var extruderIdx = 0; extruderIdx < 4; extruderIdx++){
        finalColors[0] -= ((1-this.extruderColors[extruderIdx].r) * extruderPercentage[extruderIdx]);
        finalColors[1] -= ((1-this.extruderColors[extruderIdx].g) * extruderPercentage[extruderIdx]);
        finalColors[2] -= ((1-this.extruderColors[extruderIdx].b) * extruderPercentage[extruderIdx]);
    }
    this.currentColor = new BABYLON.Color4(finalColors[0],finalColors[1],finalColors[2],1);
    break;
  }
 } else {
...
processor.prototype.createScene = function(scene){
...
var lineMesh = BABYLON.MeshBuilder.CreateLineSystem("lines", {lines: lineArray, colors: colorArray, updatable: false, useVertexAlpha: false}, scene);
/*
lineMesh.enableEdgesRendering();
lineMesh.edgesWidth = 50;
lineMesh.edgesColor = new BABYLON.Color4(1,1,1,1);
*/
var travelArray = [];
...
var travelMesh = BABYLON.MeshBuilder.CreateLineSystem("travels", {lines: travelArray, colors: travelColorArray, updatable: false, useVertexAlpha: false}, scene);
...
if(ver === 5){
	...
	var cylinder = BABYLON.MeshBuilder.CreateBox("box", {width:1, height:0.3, depth:0.6}, scene);
