...
//
// Fetches the `DuetAPI.xml` documentation file from the connected machine, caches the parsed Document, and exposes
// lookup helpers for the object-model browser plugin and the Monaco-editor hover provider. Centralising both the
// download and the path-to-`<member>` resolution keeps the two call sites in lockstep: any mangling tweak (e.g.
// when RRF adds a new virtual property) applies everywhere.

import { getErrorMessage } from "@/utils/errors";
import store from "@/store";

// Regex-based transforms mapping runtime object-model paths onto the C# property names produced by the XML doc
// compiler. The compiler drops the type segment (e.g. `ObjectModel.Move`) so we have to infer it from context.
...
/**
 * Kick off (or return the in-flight) load of `DuetAPI.xml`. Callers can await this before treating `getDuetApiDocument`
 * as authoritative; subsequent calls resolve immediately against the cached Document.
 */
export function loadDuetApi(): Promise<Document | null> {
	...
	if (loading === null) {
		loading = (async () => {
			try {
				const content: string = await store.dispatch("machine/download", {
...
/**
 * Look up the `<member>` XML element that documents a given object-model path. Accepts both literal paths
 * (`move.extruders[0].pressureAdvance`) and the normalised form used by MonacoTokens (`move.extruders[].pressureAdvance`);
 * the latter is rewritten to `[0]` so the propertyAdjustments regexes fire consistently. Returns null if no entry
 * matches, mirroring the multi-strategy lookup the object-model browser plugin has always used.
 */
export function lookupApiMember(doc: Document, path: string): Element | null {
	...
	// Build a list of candidate names to try, progressively dropping leading segments so a deeply-nested path falls
	// back to the shortest-matching `<member>` (mirrors the collapsed-type naming the XML compiler emits).
...
/**
 * Pull a doc tag (`summary` / `remarks`) out of a `<member>` XML element and normalise it for HTML rendering:
 * strip whitespace, convert newlines to `<br>`, and flatten the C# doc compiler's `<see cref="P:..."/>` refs to
 * plain property paths.
 */
...
/**
 * Convenience lookup used by the MonacoTokens hover callback. Returns a combined HTML blob (summary, optional
 * remarks) or null if the path isn't documented. Lazily kicks off the XML download on the first call so hovers
 * done before the editor finishes preloading still get a populated tooltip once the fetch completes (the hover
 * provider awaits the returned Promise). The hover provider embeds the result into a Monaco IMarkdownString with
 * `supportHtml: true`, so returning raw HTML is fine.
 */
